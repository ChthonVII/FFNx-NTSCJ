...
// SRGB to rec2020 (linear RGB input, linear RGB output):
...
vec3 CRTSimulation(vec3 rgb_input){
	...
	// linearize with a slightly modified version of BT1886 Appendix 1 EOTF function
  // (we need to that accept input >1.0 due to not clamping the color correction circuit)
	// add B
...
// - input is CRT gamma-space R'G'B'
// - stored values are gamma-space sRGB
...
vec3 GamutLUT(vec3 rgb_input)
{
	...
	// The input is in CRT gamma space, so we need to get it, and the LUT indices, into linear RGB to compute interpolation weights.
